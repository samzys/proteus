...
import proteus.gwt.shared.graphics.sun.geom.Crossings;
import proteus.gwt.shared.graphics.sun.geom.Curve;
...
public final class GeneralPath implements Shape, Cloneable {
	...
	public Object clone() {
		 ...
		 * pointCoords.clone(); return copy; } catch (CloneNotSupportedException
		 * e) { // this shouldn't happen, since we are Cloneable throw new
		 * InternalError(); }
		 */
		GeneralPath copy = new GeneralPath(this.getWindingRule(), numTypes,
				numCoords);
		copy.numTypes = numTypes;
		copy.numCoords = numCoords;
		for (int i = 0; i < copy.numTypes; ++i) {
			copy.pointTypes[i] = pointTypes[i];
		}
		for (int i = 0; i < copy.numCoords; ++i) {
			copy.pointCoords[i] = pointCoords[i];
		}
		return copy;
	}
